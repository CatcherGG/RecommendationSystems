...
from .nar_utils import load_acr_module_resources, load_thumbnail_embaddings,load_nar_module_preprocessing_resources, save_eval_benchmark_metrics_csv, \
...
tf.flags.DEFINE_string('load_thumbnail_embaddings_path',
                       default='/pickles', help='thumbnail embaddings path')
...
def main(unused_argv):
    try:
        ...
        tf.logging.info('Loading thumbnail emmbaddings')
        thumbnail_embaddings = load_thumbnail_embaddings(FLAGS.load_thumbnail_embaddings_path)
        ...
        tf.logging.info('Creating new content article embeddings')
        zero_arr = numpy.zeros((list(thumbnail_embaddings.values()))[0].shape)
        article_id_to_index_dict = acr_label_encoders['article_id']
        print(article_id_to_index_dict)
        inverted_dict = {v: k for k, v in article_id_to_index_dict.items()}
        new_emmbadding_matrix = np.zeros([len(articles_metadata_df), len(zero_arr)+250])
        print(new_emmbadding_matrix.shape)

        for row in articles_metadata_df.itertuples(index=True):
            if inverted_dict[row.article_id] in thumbnail_embaddings.keys():
                new_emmbadding_matrix[row.article_id-1] = numpy.concatenate([content_article_embeddings_matrix[row.article_id], thumbnail_embaddings[inverted_dict[row.article_id]]])
            else:
                new_emmbadding_matrix[row.article_id-1] = numpy.concatenate([content_article_embeddings_matrix[row.article_id], zero_arr])
        content_article_embeddings = new_emmbadding_matrix
        ...
        content_article_embeddings_matrix = content_article_embeddings_with_padding
        tf.logging.info("new content embeddings shape: {}".format(content_article_embeddings_matrix.shape))
